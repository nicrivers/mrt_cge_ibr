...
parameters
	TofT(fmt,*,*)		Terms-of-trade welfare change (denote by the numeraire of the acting region),
...
loop(rm(r),
        ...
        gdp("abs","expend","X-M",r,%1)		= -vb(r)*(PFX$rx(rnum)+PD("i",rnum)$rm(rnum))/pnum(r);
...
*.	vadd(g,"revtfi",r)		=  sum(i,REV_TFI(i,g,r))/pnum(r);
        ...
        inc("abs","vb",r,%1)	   = -vb(r)*(PFX$rx(rnum)+PD("i",rnum)$rm(rnum))/PD("c",r);
...
*.transfers("abs",r,%1)			  = sum(s, TRNSF(s)*trnv(r,s)*PD("c",rnum))/pnum(r);
	...
	transfers("abs","all",%1)		  = sum(s, transfers("abs",s,%1));


	TofT("abs",r,%1)$(not rcalc(r))		   = RA(r)/PD("c",r) - RA_BMK(r) ;
	TofT("abs",rcalc(r),%1)			   = - sum(s$(not rcalc(s)), RA(s)/PD("c",s) - RA_BMK(s) );
	...
	summary("abs","Emissions","non-EITE+FD",r,%1)	= quants("abs","CO2","non-EITE+FD",r,%1);	
...
summary("abs","Welfare","all",r,%1)	   = RA.L(r)/PD.l("c",r);