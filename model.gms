...
*	Note:	The sensitivity analysis on structural parameters such as elasticities are moved into scen.gms
*		where they can be controlled from the outset via the batch command line (or ssa.gms). This is fine, 
*		as long as the parameteric changes do not affect the benchmark economy.If we were however to compute
*		a new benchmark -- e.g. by setting all initial taxes to zero to obtain a first-best reference BMK --
*		then we would have to re-assign elasticities in model.gms and recalibrate explicitly again.
...
parameter byr_gdp	Base year GDP (in billion USD);
...
parameters
        ...
        esub_e(g,r)	Elasticity of substitution within energy composite;	
...
display esubm, esubd;
*	The GTAP dataset features unrealistically high intra-import elasticity of substitution for gas (esubm(gas) = 32.579)
...
$sectors:
	Y(g,r)$(rm(r)*vom(g,r))			! Output
	DX(g,r)$(rm(r)*vom(g,r))		! Transformation of output into domestic and export supply
	M(i,r)$(rm(r)*vim(i,r))			! Imports
	YT(j)$vtw(j)				! Transportation services
	A(i,r)$(rm(r)*avafm(i,r))		! Armington supply
	X(i,r)$vem(i,r)				! Exports to or from rest of world (SOE model)
	CO2(g,r)$(rm(r)*co2q(g,r))		! CO2 emission flows

$commodities:
	PY(g,r)$(rm(r)*vom(g,r))		! Output price
	PD(g,r)$(rm(r)*(vom(g,r)-vxm(g,r)))	! Domestic supply price
	PM(i,r)$(rm(r)*vim(i,r))		! Import price
	PE(g,r)$(rx(r)*vem(g,r)+rm(r)*vxm(g,r))	! Export price
	PT(i)$vtw(i)				! Price for Transportation services
	PF(f,r)$(rm(r)*mf(f)*evom(f,r))		! Mobile primary factors rent
	PS(f,g,r)$(rm(r)*sf(f)*vfm(f,g,r))	! Sector-specific primary factors
	PA(i,r)$(rm(r)*avafm(i,r))		! Armington supply price 
	PCO2(g,r)$(rm(r)*co2q(g,r))		! CO2 emission price by sector and region
	PCO2R(r)$(rm(r)*sum(g, rtax(g,r)))	! Regional CO2 emission price
...
$auxiliary:
	PHI(r)$(rm(r)*ptgt(r))			! Rationing of CO2 budget to hit exogenous CO2 price
	RHO(g,r)$rm(r)				! Tax on emission inputs
	PSI(g,r)$rm(r)				! Subsidy on production output
	TAU$glbtgt				! Rationing of emission budget for leakage compensation
	TRNSF(r)$rm(r)				! Compensating transfer
	RS(xe,r)$(rm(r)*ks0("res",xe,r))	! Regional energy resource supply

*	Transformation of output into domestic and export supply
$prod:DX(g,r)$(rm(r)*vom(g,r))	t:etadx(g) 
	o:PD(g,r)	q:(vom(g,r)-vxm(g,r))	
	o:PE(g,r)	q:vxm(g,r)		
	i:PY(g,r)	q:vom(g,r)

*	Production (except for fossil fuels)
$prod:Y(g,r)$(rm(r)*vom(g,r)*(not xe(g))) 
...
$prod:Y(g,r)$(rm(r)*vom(g,r)*xe(g)) s:esubr(g,r)  id:0
...
$prod:YT(j)$vtw(j)  s:1
	...
	i:PD(j,r)$(rm(r)*(not vxm(j,r))) q:vst(j,r)
...
+		a:RA(rm) t:(-rtxs(i,rm,r)) a:RA(r) t:(rtms(i,rm,r)*(1-rtxs(i,rm,r)))
	i:PD(i,rm)$(not vxm(i,rm))	q:vxmd(i,rm,r)	p:pvxmd(i,rm,r) rm.tl:
+		a:RA(rm) t:(-rtxs(i,rm,r)) a:RA(r) t:(rtms(i,rm,r)*(1-rtxs(i,rm,r)))
...
*	Armington composite
$prod:A(i,r)$(rm(r)*avafm(i,r)) s:esubd(i)
...
$prod:X(i,r)$(rm(r)*vem(i,r))
	...
	i:PD(i,r)$(not vxm(i,r))	q:vem(i,r)	a:RA(r)	t:(-rtxs_row(i,r))
...
$demand:ROW$card(rx)
	e:PFX				q:rowpfx
	e:PD("i",rnum)$rm(rnum)		q:(-sum(rm,vb(rm)))
...
$constraint:RHO(g,r)$(rm(r)*(bcr(r) and eite(g))) 
...
$constraint:TAU$glbtgt
	glbtgt =e= sum(r$rm(r), sum(g$co2q(g,r), CO2(g,r))); 

*	Compensating transfers
$constraint:TRNSF(r)$rm(r)
	Y("c",r) =e= ctrg(r);

*       Multiplicative scaling of fossil fuel resources to achieve a given world price for heterogeneous energy goods
$constraint:RS(xe,r)$(rm(r)*ks0("res",xe,r))
	ptarget(xe,r)*PD("c",rnum) =e= PE(xe,r);

$report:
	v:V_AFM(i,g,r)$(rm(r)*vafm(i,g,r))                      i:PA(i,r)	prod:Y(g,r)
	v:V_FM(f,g,r)$(rm(r)*sf(f)*vom(g,r)*vfm(f,g,r))         i:PS(f,g,r)	prod:Y(g,r)
	v:V_FM(f,g,r)$(rm(r)*mf(f)*vom(g,r)*vfm(f,g,r))         i:PF(f,r)	prod:Y(g,r)
	v:V_VST(i,r)$(rm(r)*vst(i,r)*(not vxm(i,r)))            i:PD(i,r)	prod:YT(i)
	v:V_VST(i,r)$(rm(r)*vst(i,r)*vxm(i,r))                  i:PE(i,r)	prod:YT(i)
	v:V_VDM(g,r)$(rm(r)*(vom(g,r)-vxm(g,r)))		o:PD(g,r)	prod:DX(g,r)
	v:V_VXM(g,r)$(rm(r)*vom(g,r)*vxm(g,r))			o:PE(g,r)	prod:DX(g,r)
	v:V_XMD(i,s,r)$(rm(r)*vxmd(i,s,r)*vxm(i,s))		i:PE(i,s) 	prod:M(i,r)
	v:V_DFM(i,r)$(rm(r)*sum(g, vdfm(i,g,r)))		i:PD(i,r)	prod:A(i,r)
	v:V_IFM(i,r)$(rm(r)*sum(g, vifm(i,g,r)))		i:PM(i,r)	prod:A(i,r)
	v:V_XMD(i,s,r)$(rm(r)*vxmd(i,s,r)*(not vxm(i,s)))	i:PD(i,s) 	prod:M(i,r)
$offtext
...
$macro REV_TO(g,r)	((rto(g,r)*PY.L(g,r)*Y.L(g,r)*vom(g,r))$(rm(r)*vom(g,r)))		
...
$macro PX(i,r)		((PD.L(i,r)$(not vxm(i,r)) + PE.L(i,r)$vxm(i,r))$rm(r) + PE.L(i,r)$rx(r))
...
parameter
	rto_(g,r)       Output (or income) subsidy rates (in %)
	rtf_(f,g,r)     Primary factor and commodity rates taxes (in %) 
	rtfd_(i,g,r)	Firms domestic tax rates (in %)
	rtfi_(i,g,r)	Firms' import tax rates (in %)
	rtxs_(i,r,s)	Export subsidy rates (in %)
	rtms_(i,r,s)	Import taxes rates (in %);

rto_(g,r)	=100*rto(g,r);   
rtf_(f,g,r) 	=100*rtf(f,g,r); 
rtfd_(i,g,r)	=100*rtfd(i,g,r);
rtfi_(i,g,r)	=100*rtfi(i,g,r);
rtxs_(i,r,s)	=100*rtxs(i,r,s);
rtms_(i,r,s)	=100*rtms(i,r,s);

execute_unload 'bmk_taxes.gdx', rto_, rtf_, rtfd_, rtfi_, rtxs_, rtms_;
$onecho >gdxxrw.txt
par=rto_   rng=rto!a2  cdim=0 intastext=n
par=rtf_   rng=rtf!a2  cdim=0 intastext=n
par=rtfd_  rng=rtfd!a2 cdim=0 intastext=n
par=rtfi_  rng=rtfi!a2 cdim=0 intastext=n
par=rtxs_  rng=rtxs!a2 cdim=0 intastext=n
par=rtms_  rng=rtms!a2 cdim=0 intastext=n
$offecho
execute 'gdxxrw i=bmk_taxes.gdx o=bmk_taxes.xlsx @gdxxrw.txt';
