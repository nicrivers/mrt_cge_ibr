...
$if not set ds		$set ds emf36
$if not set yr		$set yr 14
$if not set gtapv	$set gtapv 10Y14
$if %yr%==2014 $set yr 14

*	Data directory is gamsdata\gtap<ver>\20<yr>\%ds%

$if not set datadir $set datadir "%system.fp%gamsdata%fs%gtap%gtapv%%fs%20%yr%%fs%"
...
$if not set thetadx $setglobal thetadx 0.5
...
set	rm(r)		Regions included in the model
