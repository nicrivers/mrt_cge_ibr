...
dat <- read_csv("master_results.csv")

# max price
max_pco2 = 250

# For main results keep arm and esube at ref levels
dat <- dat %>%
  filter(arm == "ref",
         esube == "ref") 

# For main results keep MRT version of results
dat <- dat %>%
  filter(mdl == "mrt")

# Keep relevant variables
dat <- dat %>%
  select(item, sector, region, policy, value, co2p, scc, region, region_implementing)

# Convert policy to factor
dat <- dat %>%
         mutate(policy = factor(policy, levels = c("LS", "O", "A", "IO", "IE")))
...
dat_soe <- read_csv("master_results.csv")
...
# For ToT analsyis, keep the SOE version of results
dat_soe <- dat_soe %>%
  filter(mdl == "soe")
...
## Now evaluate terms of trade
tot <-
  dat %>%
  filter(item == "welfare$",
         scc == 250) %>%
  rename(welfare_mrt = value) %>%
  inner_join(
    dat_soe %>%
      filter(item == "welfare$",
             region == region_implementing) %>%
      rename(welfare_soe = value)
  ) %>%
  mutate(tot_effect = welfare_mrt - welfare_soe) %>%
  arrange(region_implementing,policy, co2p) %>%
  select(region_implementing, policy, co2p, tot_effect)
...
tot %>%
  # Join with original data to calculate percent changes
  inner_join(dat %>%
               filter(item == "welfare$",
                      scc == 250,
                      region == region_implementing) %>%
               arrange(region, policy, co2p) %>%
               group_by(region_implementing) %>%
               summarise(base_welfare=first(value))) %>%
  mutate(tot_effect_percent = tot_effect / base_welfare) %>%
  filter(co2p <= max_pco2) %>%
...
dat <- read_csv("master_results.csv") %>%
  filter(mdl == "mrt") %>%
# Keep relevant variables
  select(item, sector, region, policy, value, co2p, scc, region, region_implementing, arm, esube) %>%
# Convert policy to factor
  mutate(policy = factor(policy, levels = c("LS", "O", "A", "IO", "IE"))) %>%
# lower case
  mutate(region_implementing = toupper(region_implementing),
...
# Set factor levels so graphs read logically
dat$arm <- factor(dat$arm, levels = c("lo", "ref", "hi"))
dat$esube <- factor(dat$esube, levels = c("lo", "ref", "hi"))

pd <- dat %>% 
  filter(item == "netwelf_globalscc") %>%
  dplyr::select(region_implementing,policy, nw=value, co2p, scc, arm, esube)

pd %>% filter(policy == "O") %>%
  inner_join(
    pd %>%
      filter(policy != "O") %>%
      group_by(region_implementing, scc, co2p, arm, esube) %>%
      summarise(welf_non_o=max(nw))
  ) %>%
  group_by(region_implementing,scc, co2p) %>%
  # Only keep where scc >= co2p
  ...
  distinct() %>%
  mutate(diff = welf_non_o-nw) %>%
  ggplot(aes(x=scc, y=co2p, z=diff, colour=arm, linetype=esube)) +
  geom_contour(breaks = c(-1,0,1)) +
  theme_light() +
  facet_wrap(~region_implementing) +
  scale_x_continuous(name="Social cost of carbon ($/t CO2)") +
  scale_y_continuous(name="Domestic carbon price ($/t CO2)") 
ggsave("figures/OBRvsothers_sensitivity.png", width = 8, height = 8)

# Alternatively, try creating the gridded figure
# Ranking table
dat %>%
  filter(item %in% c("CO2 price", "netwelf_globalscc"),
         sector == "all",
  ...
  filter(region == "USA") %>%
  pivot_wider(names_from=item, values_from=value) %>% 
  group_by(region_implementing,co2p, scc, arm, esube) %>% 
  arrange(co2p, scc, desc(netwelf_globalscc)) %>% 
  summarise(best = first(policy), 
            second = nth(policy,2), 
            third = nth(policy,3), 
  ...
  ggplot(aes(x=scc, y=co2p, fill=best, label=best)) +
  geom_raster(show.legend=TRUE) +
  facet_wrap(~arm+esube) +
  scale_fill_brewer(name="policy",palette = "Set1", drop=FALSE) +
  theme_light() +
  scale_x_continuous(name="Social cost of carbon ($/t CO2)") +
  scale_y_continuous(name="Domestic carbon price ($/t CO2)")
ggsave("figures/optimalpolicy_sensitivity_usa.png", width = 8, height = 8)
...
dat %>%
  ...
  scale_x_continuous(name="Social cost of carbon ($/t CO2)") +
  scale_y_continuous(name="Domestic carbon price ($/t CO2)")
ggsave("figures/optimalpolicy_sensitivity_eur.png", width = 8, height = 8)
